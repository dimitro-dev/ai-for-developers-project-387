...
использоваться для улучшения модели, что для публичного учебного репозитория приемлемо. Обратная связка обязательна: перевод строки `model:` на платную модель
...
Побочное наблюдение: ответ агента сам по себе является событием `issue_comment` и порождает
прогон 33306685456, где оба job'а пропущены — в теле нет `/oc`. Цикла не возникает, цена —
несколько секунд раннера на каждый ответ. Если понадобится убрать и это, условие job'а
`comment` дополняется `github.event.comment.user.type != 'Bot'`.
