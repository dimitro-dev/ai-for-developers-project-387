...
jobs:
  comment:
    ...
    steps:
      ...
      - uses: anomalyco/opencode/github@a3b97d9090ccf4aa9ac32268486283e3131e36b4
        env:
...
`anomalyco/opencode/github` закреплён по SHA коммита тега `github-v1.2.25`
(`a3b97d9090ccf4aa9ac32268486283e3131e36b4`, сверено `git ls-remote` от 2026-08-30), а не по
плавающему `@latest`. Причина — supply-chain, а не удобство: действие третьего репозитория,
и версия, приходящая на раннер, обязана быть воспроизводимой между прогонами. `@latest` для
этого действия устаревает — тег обновляется вручную шагом публикации и отставал от выпусков
(верхний issue репозитория `anomalyco/opencode#19865` «@latest is stale; stuck on github-v1.2.9»);
на момент фиксации этого решения `@latest` указывал на `github-v1.2.19`, хотя выпущен
`github-v1.2.25`. Обновление действия — осознанная правка SHA под новый тег `github-vX.Y.Z`,
правила workflow при этом не меняются.

Ограничение честности закрепления: SHA фиксирует код действия, но бинарь CLI действие ставит
на раннере само, из своего ресурса, всегда последний; отдельного входа `version` в action.yml
`github-v1.2.25` ещё нет. То есть закреплён обвязочный код, а не интерпретатор — это уменьшает
поверхность чужого кода между прогонами, но не сводит её к нулю.
...
Агент подключён и проверен на обоих триггерах. Работа шла тремя итерациями: PR #3 —
workflow и авто-ревью, PR #5 — замена модели и фильтр ботов, PR #6 — закрытие статусов.
Приёмка задания — прогон 33307229361.

### Фиксация стороннего действия — закрыта правкой по issue #9

Собственное авто-ревью и разбор issue #9 вскрыли: `anomalyco/opencode/github@latest` — стороннее
действие по плавающему тегу. По решению выше `@latest` заменён на SHA
`a3b97d9090ccf4aa9ac32268486283e3131e36b4` (коммит тега `github-v1.2.25`) в обоих job'ах
`opencode.yml`. Факты сверены `git ls-remote` от 2026-08-30: `@latest` указывал на `github-v1.2.19`
(тег `latest` обновляется вручную шагом публикации действия), выпущен `github-v1.2.25`, — то есть
закрепление попутно подтянуло действие на актуальную версию. Гейт `setup` возвращён в черновик
(`scripts/task draft infra/010 setup`) и требует повторного согласования вместе с `result`.

Проверки: PR с этой правкой не-draft — job `review` обязан стартовать на нём и исполнить уже
закреплённую версию действия; комментарий `/oc` из issue #9 запускает job `comment` на той же
версии. Локально: `lint-docs` и `task-check` — exit 0; полный `make gates` падал один раз на
таймауте клиентского Jest (GuestBookingForm), повторный прогон той же зоны зелёный — к правке
не относится.
...
конфигурации. Доработкой к этому дню стороннее действие закреплено по SHA (issue #9) —
дефект закрыт.
